...
.text
	...
	Initialization:
		...
		li	$t8, 0		# t8 will be the delay counter, we decrement this as the aliens die, so they travel faster
...
#
	...
	iteratePixels:
		...
		ifWhite:
			...
			blt	$t6, $t8, iterate	# Don't update blue if timer isn't ready
			
			#lw	$t3, end
			#slt 	$t3,$t3,$t0     	# checks if $t0 (current pixel location) > $t3 
			#beq 	$t3,1,gameOverInit   	# if $t0 > $t3, goes to gameOver 
			bgt	$t0, 0x10000d80, gaymOver
